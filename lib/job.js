...
Job.prototype.run = function (activeThreadsCount, callback) {
    ...
    self._save({'pid':child.pid, status: self.nconf.get('statusAlias:running')});
...
Job.prototype._finish = function (code) {
    if (code == 0) {
        this._save({status: this.nconf.get('statusAlias:success'), finished: new Date().getTime()/1000});
        this.logger.info('THREAD ' + this.threadName + ': done with SUCCESS');
    } else {
        this._save({status: this.nconf.get('statusAlias:error'), finished: new Date().getTime()/1000});
