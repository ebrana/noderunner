...
Job.prototype.isDue = function (checkIntervalStart, checkIntervalEnd) {
    ...
    if (parsedSchedule && parsedSchedule[1]) {
        ...
        var offset = Math.round(randomGenerator.integer({min:0, max:(minutesInterval-1)*100})*0.6);
    } else {
        var offset = 0;
    }
    ...
    if (checkIntervalStart < nextWithOffset) {
        if (nextWithOffset <= checkIntervalEnd) {
            // inside of interval
            this.logger.debug('✓ job ' + this.document._id + ' with next due time ' + time(nextWithoutOffset)+'-' + offset + 's='+time(nextWithOffset)+' is inside of check interval: ('+time(checkIntervalStart)+'..['+time(nextWithOffset)+']..'+time(checkIntervalEnd)+'>');
        } else {
            // after interval
            this.logger.debug('✗ job ' + this.document._id + ' with next due time ' + time(nextWithoutOffset)+'-' + offset + 's='+time(nextWithOffset)+' is after check interval: ('+time(checkIntervalStart)+'..'+time(checkIntervalEnd)+'>  '+(nextWithOffset-checkIntervalStart)+'s  ['+time(nextWithOffset)+']');
        }
    } else {
        // before interval
        this.logger.debug('✗ job ' + this.document._id + ' with next due time ' + time(nextWithoutOffset)+'-' + offset + 's='+time(nextWithOffset)+' is before check interval: ['+time(nextWithOffset)+']  '+(checkIntervalStart - nextWithOffset)+'s  ('+time(checkIntervalStart)+'..'+time(checkIntervalEnd)+'>');
    }
