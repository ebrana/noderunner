...
Job.prototype.isDue = function () {
    // load distribution algorithm - for example when using */10 * * * * schedule, every job should run in any minute in interval 0..9 (but every job every time with the same offset)
    var checkInterval = this.nconf.get('planned:interval')/60000;
    var parsedSchedule = /\*\/(\d*)( \*){4}/.exec(this.document.schedule);
    if (parsedSchedule && parsedSchedule[1]) {
        var minutesInterval = parsedSchedule[1];

        // find random number derieved from job id in interval (0..minutesInterval)
    ...
    var diff = (now.valueOf() - (next.valueOf() - offset)) / 60000;

    function time(date) {
        return new Date(date).getHours() + ':' + new Date(date).getMinutes() + ':' + new Date(date).getSeconds();
    }

    this.logger.debug('checking planned job ' + this.document._id + ' with offset=' + offset / 60000 + ' and test diff if <'+Math.round(-checkInterval*100/2)/100+'..'+Math.round(diff*100)/100+'..'+Math.round(checkInterval*100/2)/100+')  (now ' + time(now.valueOf()) + ', next ' + time(next.valueOf()) + ', due ' + time(next.valueOf() - offset) + ')');

    return -checkInterval/2 <= diff && diff < checkInterval/2;
}
