...
Job.prototype.isDue = function () {
    ...
    if (parsedSchedule && parsedSchedule[1]) {
        ...
        var offset = randomGenerator.integer({min:0, max:minutesInterval-1})*60000;
    } else {
        var offset = 0;
    }

    // next() returns next due time, so we need to move time back by one minute to get current due time
    var now = new Date();
    var next = parser.parseExpression(this.document.schedule, {currentDate: now.valueOf() - 60000}).next();
    now.setSeconds(0);

    if (offset > 0) {
        this.logger.debug('checking planned job ' + this.document._id + ' with offset=' + offset / 60000 + ' and diff=' + (now.valueOf() - (next.valueOf() - offset)) / 60000 + ' (now ' + new Date(now.valueOf()).getHours() + ':' + new Date(now.valueOf()).getMinutes() + ', next ' + new Date(next.valueOf()).getHours() + ':' + new Date(next.valueOf()).getMinutes() + ', due ' + new Date(next.valueOf() - offset).getHours() + ':' + new Date(next.valueOf() - offset).getMinutes() + ')');
    }

    return now.valueOf() == next.valueOf() - offset;
}
