...
Job.prototype.isDue = function (checkIntervalStart, checkIntervalEnd) {
    ...
    if (parsedSchedule && parsedSchedule[1]) {
        ...
        var offset = Math.round(randomGenerator.integer({min:0, max:(minutesInterval-1)*100})*600);
    } else {
        var offset = 0;
    }
    ...
    var now = new Date();
    var next = parser.parseExpression(this.document.schedule, {currentDate: now.valueOf() - this.nconf.get('planned:interval')}).next();
    var nextWithOffset = next.valueOf() - offset;
    ...
    if (checkIntervalStart <= nextWithOffset) {
        if (nextWithOffset < checkIntervalEnd) {
            // inside of interval
            this.logger.debug('✓ job ' + this.document._id + ' with next due time ' + time(next.valueOf())+'-' + offset / 60000 + '='+time(nextWithOffset)+' is inside of check interval ('+time(checkIntervalStart)+'..['+time(nextWithOffset.valueOf())+']..'+time(checkIntervalEnd)+'>');
        } else {
            // before interval
            this.logger.debug('✗ job ' + this.document._id + ' with next due time ' + time(next.valueOf())+'-' + offset / 60000 + '='+time(nextWithOffset)+' is after check interval ('+time(checkIntervalStart)+'..'+time(checkIntervalEnd)+'>  '+(nextWithOffset-checkIntervalStart)/1000+'s  ['+time(nextWithOffset.valueOf())+']');
        }
    } else {
        // after interval
        this.logger.debug('✗ job ' + this.document._id + ' with next due time ' + time(next.valueOf())+'-' + offset / 60000 + '='+time(nextWithOffset)+' is before check interval ['+time(nextWithOffset.valueOf())+']  '+(checkIntervalStart - nextWithOffset)/1000+'s  ('+time(checkIntervalStart)+'..'+time(checkIntervalEnd)+'>');
    }
...
Job.prototype._finish = function (code, callback, fallback) {
    ...
    if (code == 0) {
        this._save({status: this.nconf.get('statusAlias:success'), finished: finished}, callback, fallback);
        this.logger.info('THREAD ' + this.threadName + ' ' + this.queue.getThreadsInfo(this.threadIndex) + '  -> ' + this.document._id + ' done with SUCCESS');
    } else {
        this._save({status: this.nconf.get('statusAlias:error'), finished: finished}, callback, fallback);
