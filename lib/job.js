...
Job.prototype.run = function (threadIndex, callback) {
  var self = this;
  var command = this._buildCommandArray();
  this.threadName = this._buildThreadName(threadIndex);
  this.threadIndex = threadIndex;

  this.logger.info('THREAD ' + this.threadName +' ' +this.queue.getThreadsInfo(threadIndex)+': running '+/*self.toString()*/this.document._id);
  ...
  self._save({
    ...
    child.on('close', function (code) {
      var duration = self._finish(code);
...
Job.prototype._finish = function (code) {
  var finished = Math.floor(new Date().getTime() / 1000);
