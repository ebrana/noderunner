...
Planned.prototype.checkRunningJobWithSource = function(id, callback) {
    var self = this;
    ...
    self.db.collection('planned').find().toArray(function (err, docs) {
        if (typeof docs !== 'undefined' && docs.length > 0) {
            ...
            docs.forEach(function (doc) {
                ...
                if (job.isDue()) {
                    // pokud je concurrenceMode skip, pak kopirujeme jen pokud neni bezici job se stejnym zdrojem
                    switch (doc.concurrenceMode) {
                        case 'skip':
                            someDueJob = true;
                            self.checkRunningJobWithSource(doc._id, function(isRunning){
                                if (isRunning) {
                            ...
                            break;
                        case 'kill':
                            // TODO najit bezici procesy se stejnym zdrojem, a nejprve je killnout
