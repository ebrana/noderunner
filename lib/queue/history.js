...
History.prototype.getJobs = function(callback) {
    ...
    this.db.collection('history').find({}, {"limit":"100", "sort": [['finished','desc']]}).each(function(err, doc) {
        if(doc == null) {
            callback(data);
            return;
        }
...
History.prototype.clean = function (hoursCount) {
    ...
    self.db.collection('history').deleteMany({finished:{$lt: lowerThanTime}}, function(err, result){
        if (err) {
            ...
            self.logger.verbose('HISTORY: clean finished<'+lowerThanTime+' success - ' + result.deletedCount + ' deleted');
        }
    });
}
