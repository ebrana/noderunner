...
History.prototype.run = function () {
    ...
    self.logger.info('HISTORY: moving old jobs from immediate queue to history');
...
History.prototype.clean = function (hoursCount) {
    ...
    self.db.collection('history').deleteMany({finished:{$lt: lowerThanTime}}, function(err, result){
        if (err) {
            ...
            self.logger.verbose('HISTORY: clean finished<'+lowerThanTime+' ('+self.nconf.get('history:cleanHours')+' hours) success - ' + result.deletedCount + ' deleted');
        }
    });
}
