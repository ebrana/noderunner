...
History.prototype.run = function () {
    ...
    self.db.collection('history').find({status:{$in:[this.nconf.get('statusAlias:success'),this.nconf.get('statusAlias:error')]}}).toArray(function (err, docs) {
        if (docs.length > 0) {
            docs.forEach(function (doc) {
                var job = new Job(self.db);
                job.initByDocument(doc);
                //job.moveToHistory();
                self.logger.verbose('HISTORY: moving job '+job._id);
            })
        } else {
