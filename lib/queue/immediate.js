...
Immediate.prototype._removeStuckRunningJobs = function(evenWithoutPid) {
    var self = this;
    self.logger.warn('IMMEDIATE: try to move stuck running/fetched jobs - evenWithoutPid=', evenWithoutPid);
    self.db.collection('immediate').find({$or:[{status:"running"},{status:"fetched"}]}).each(function(err, doc){
        if (doc != null) {
            if ((evenWithoutPid && !doc.pid) || (doc.pid && !isRunning(doc.pid)) ) {
                ...
                self.logger.warn('IMMEDIATE: moving stuck running/fetched job with PID and _id: ', doc.pid, doc._id.toString());
                delete doc._id;
                doc.status = "stucked";
