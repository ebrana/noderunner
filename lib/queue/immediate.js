...
Immediate.prototype._check = function () {
    ...
    self.fetchNextJob(function (job) {
        ...
        self.emit('runningCountChanged', self.getRunningThreads().length);

        // job found - save stats and run immediately
        self.createJobStat(job, threadIndex);
        job.run(threadIndex, function () {
            ...
            self.logger.error('IMMEDIATE: error during job run, sleep for ' + self.nconf.get('immediate:interval')/1000 + ' secs');
            clearTimeout(self.timeout);
...
Immediate.prototype.resetJobStats = function() {
    this.jobStats = {};
}
