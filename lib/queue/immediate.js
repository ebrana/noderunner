...
Immediate.prototype.getJobs = function(callback) {
    ...
    this.db.collection('immediate').find().each(function(err, doc) {
        if(doc == null) {
            callback(data);
            return;
        }
...
Immediate.prototype._removeStuckRunningJobs = function() {
    var self = this;
    var documentsToMove = self.db.collection('immediate').find({"status":"running"}).each(function(err, doc){
        if(doc != null) {
            self.db.collection('immediate').remove(doc);
            delete doc._id;
            doc.status = "stucked";
            self.db.collection('history').insert(doc);
            self.logger.warn('moving stuck running job');
        }
    });
}
