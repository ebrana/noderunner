...
Immediate.prototype._removeStuckRunningJobs = function(evenWithoutPid) {
    var self = this;
    self.logger.warn('IMMEDIATE: try to move stuck running/fetched job with');
    self.db.collection('immediate').find({$or:[{status:"running"},{status:"fetched"}]}).each(function(err, doc){
        if (doc != null) {
            if ((!doc.pid && evenWithoutPid) || !isRunning(doc.pid)) {
                self.db.collection('immediate').remove(doc);
                self.logger.warn('IMMEDIATE: moving stuck running/fetched job with PID ', doc.pid, doc._id.toString());
                delete doc._id;
                doc.status = "stucked";
...
Immediate.prototype._check = function () {
    ...
    self.fetchNextJob(threadIndex, function (job) {
        ...
        job.run(function () {
            ...
            // plan stuck jobs control after one hour - should be enough for mongo to be up again
            setTimeout(function () {
