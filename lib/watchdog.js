...
Watchdog.prototype._calculateLoad = function(forSeconds, callback) {
    ...
    self.db.collection('history').find({
        ...
        callback(totalUsedSeconds/intervalDuration + '/' + self.immediateQueue.threads.length + ' (' + Math.round(self.immediateQueue.threads.length/(totalUsedSeconds/intervalDuration)) + ' %)');
    });
