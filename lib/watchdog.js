...
Watchdog.prototype._runCongestionCheck = function() {
    ...
    if (sinceLastCheck > self.nconf.get('immediate:interval') * 3) {
        ...
        if (!self.email2Sent) {
            self._sendEmail('Time since last immediate queue check is more than 3 times greater than set check interval ('+sinceLastCheck+'>'+(self.nconf.get('immediate:interval')*3)+')! Immediate queue is probably not working.', function () {
                self.email2Sent = true;
            });
        }
    }
...
Watchdog.prototype._runThreadsStatsCheck = function() {
    var self = this;
    var threadStat = this._calculateThreadsStat();
    ...
    self.logger.verbose('WATCHDOG: average load for last '+interval+'s: '+threadStat.total+' ['+threadStat.byThread.join(',')+']');
}

Watchdog.prototype.loadThreadsStats = function(callback) {
    var secondsBack = 12 * 60 * 60;
    this.db.collection('load').find({"intervalTo":{$gt: (Date.now()/1000-secondsBack) }}).toArray(function (err, data) {
        if (!err) {
            callback(data);
        }
    });
...
Watchdog.prototype._calculateThreadsStat = function() {
    var self             = this;
