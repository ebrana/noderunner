  ...
  self.db
    ...
    .toArray(function(err, docs) {
      if (typeof docs !== 'undefined' && docs && docs.length > 0) {
        ...
        var checkIntervalStart = self.lastCheckTime
          ? self.lastCheckTime
        ...
        if (dueJobsCount > 0) {
          ...
          self.logger.verbose(
            'PLANNED: no due job, sleep for ' + self.nconf.get('planned:interval') / 1000 + 's'
          )
        }
      } else {
        self.logger.verbose(
          'PLANNED: no planed job, sleep for ' + self.nconf.get('planned:interval') / 1000 + 's'
        )
      }
